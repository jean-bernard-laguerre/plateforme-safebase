...
const RestoreForm = ({history, close}: {
    history:History, 
    close: () => void
}) => {
    ...
    async function getUserDatabase() {
        ...
        if (response.success === false) {
          ...
          const filtered = response.connections.filter((db: Database) => (
    ...
    return (
        <div>
            <h2 className="text-lg font-medium mb-4">Restore {history.Bdd_source_name}</h2>
            <div className="mb-4">
                ...
                >
                    <option value={0}>
                        Selectionnez une base de donnée
                    </option>
